...
BUILDLINK_PREFIX.blas=	/usr/pkg
BUILDLINK_PREFIX.dl=	/usr
BUILDLINK_PREFIX.gettext=	/usr
BUILDLINK_PREFIX.iconv=	/usr
BUILDLINK_PREFIX.lapack=	/usr/pkg
BUILDLINK_PREFIX.libsodium=	/usr/pkg
BUILDLINK_PREFIX.pthread=	/usr
BUILDLINK_PREFIX.py-numpy=	/usr/pkg
...
_BLNK_PHYSICAL_PATH.WRKDIR=	/usr/pkgsrc/category/py-crossbar/work
_BLNK_PKG_DBDIR.blas=	/usr/pkg/pkgdb/blas-3.9.0nb1
_BLNK_PKG_DBDIR.lapack=	/usr/pkg/pkgdb/lapack-3.9.0nb1
_BLNK_PKG_DBDIR.libsodium=	/usr/pkg/pkgdb/libsodium-1.0.18
_BLNK_PKG_DBDIR.py-numpy=	/usr/pkg/pkgdb/py38-numpy-1.19.4
_BLNK_PKG_DBDIR.python38=	/usr/pkg/pkgdb/python38-3.8.6nb1
_IGNORE_INFO_PATH=	
_MANCOMPRESSED=	no
_MANZ=	no
_USE_TOOLS=	[ awk basename cat chgrp chmod chown cmp cp curl cut date diff digest dirname echo egrep env expr false fgrep find grep gzcat head hostname id install ln ls mkdir mv patch printf pwd rm rmdir sed sh sort tail tar test touch tr true uname wc xargs
