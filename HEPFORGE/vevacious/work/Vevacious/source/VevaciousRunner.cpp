...
namespace Vevacious
{
  std::string const VevaciousRunner::vevaciousVersion( "1.2.03" );
  ...
  VevaciousRunner::VevaciousRunner( std::string const& modelFilename,
    ...
    tadpoleSolver( hom4ps2Directory,
                   homotopyType ),
    potentialMinimizer(),
    pythonMinuitWrapper( pythonMinuitWrapper ),
    treeLevelExtrema(),
  ...
  {
    BOL::StringParser::transformToUppercase( this->pythonMinuitWrapper );
  ...
  VevaciousRunner::VevaciousRunner( BOL::ArgumentParser& argumentParser ) :
    ...
    tadpoleSolver( argumentParser.fromTag( "hom4ps2_dir",
                   ...
                   argumentParser.fromTag( "homotopy_type",
                                           "1" ) ),
    potentialMinimizer(),
    pythonMinuitWrapper( argumentParser.fromTag( "python_wrapper",
                                                 "PYMINUIT" ) ),
    treeLevelExtrema(),
    firstWriteOfExtrema( true ),
    ...
    rollingTolerance( BOL::StringParser::stringToDouble(
                                      argumentParser.fromTag( "roll_tolerance",
                                                              "0.1" ) ) ),
    tunnelToGlobal( argumentParser.fromTag( "tunnel_to_global",
                                          "False" ) ),
    ...
    deformTunnelPaths( argumentParser.fromTag( "deform_tunnel_paths",
                                               "True" ) )
  {
    BOL::StringParser::transformToUppercase( pythonMinuitWrapper );
...
"import scipy\n"
"import sys\n";
    if( pythonMinuitWrapper == "IMINUIT" )
    {
      outputFile << "import iminuit as minuit\n";
    }
    else if( pythonMinuitWrapper == "PYMINUIT" )
    {
      outputFile << "import minuit\n";
    }
    else
    {
      std::stringstream errorBuilder;
      errorBuilder << "Did not recognize \"" << pythonMinuitWrapper
      << "\" as a valid Python Minuit wrapper name!"
      << " It must be IMINUIT or PYMINUIT.";
...
"        self.tunnelToGlobal = " << tunnelToGlobal << "\n"
