 ...
 *      (in Vevacious/GNU_public_license.txt ) along with Vevacious. If not,  *
...
\*****************************************************************************/
 ...
 José Eliel Camargo (Elielcamargomolina@gmail.com),
...
CHANGELOG:
 * 9th July 2018: version 1.2.03
 - Fixed compatibility issues with newer versions of numpy. The issues had to 
   do with numpy.reshape expecting an array of ints.
 - Added option in initialization file for always tunneling to the global minimum.
   Previously (now set as default) the approach was to tunnel to true vacuum closest
   in field space to the DSB (input) minimum.
 - Fixed bug that made Vevacious break when calculating tunneling at a temperature
   where the panic vacuum was higher than the DSB. Instead of breaking it now searches
   for a slightly lower temperature where that is not the case anymore.
 - Updated the example SLHA files and outputs. 
